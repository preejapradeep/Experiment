            ...
            for metric in similarity_metrics:
                ...
                for bt_name, bt in case_base_dict.items():
                    ...
                    if algorithm == "Tree Edit Distance":
                        score = ted_similarity(bt,random_bt_prime, delta=std_delta)
                    elif algorithm == "Levenshtein Distance":
                        score = levenshtein_similarity(bt,random_bt_prime, delta=std_delta)
                    elif algorithm == "Affine Edit Distance":
                        score = aed_similarity(bt,random_bt_prime, delta=std_delta)
                    elif algorithm == "Dynamic Time Warping":
                        score = dtw_similarity(bt,random_bt_prime, delta=std_delta)
                    elif algorithm == "Set Edit Distance":
                        score = set_similarity(bt,random_bt_prime, delta=std_delta)
                    # elif algorithm == "Overlap Coefficient":
                    #     score = overlap_similarity(bt,random_bt_prime, delta=std_delta)
                    # elif algorithm == "Sequence Match":
                    #     score = sequence_match_similarity(bt,random_bt_prime, delta=std_delta)
                    
                    results.loc[len(results.index)] = [random_bt, edits, operation, random_bt_prime, bt, algorithm, metric_name, score]
...
if __name__ == '__main__': 
    ...
    folder_path = '/Users/ppradeep/Documents/BT Algorithm/randomgenerator/BT_Random/ModifiedBTExp_June/sample/stddelta/'
    ...
    # casebase300
    with open("100junecb.json", "r") as f:
